        ...
        <div className="border-t border-white/5 p-3 grid grid-cols-2 gap-x-4 gap-y-1.5 animate-fade-in">
          ...
          {click.latitude && click.longitude && (
            <div className="col-span-2">
              <p className="text-xs text-slate-600">Lokasi (IP Geo)</p>
              <a
          ...
          {click.gps_latitude && click.gps_longitude && (
            <div className="col-span-2">
              <p className="text-xs text-slate-600">
                📍 Lokasi GPS Akurat
                {click.gps_accuracy && (
                  <span className="text-slate-700 ml-1">(±{Math.round(click.gps_accuracy)}m)</span>
                )}
              </p>
              <a
                ...
                rel="noopener noreferrer"
                onClick={e => e.stopPropagation()}
                className="text-xs text-emerald-400 hover:text-emerald-300 font-mono transition-colors font-medium"
              >
                {click.gps_latitude.toFixed(6)}, {click.gps_longitude.toFixed(6)} → Buka Maps ↗
              </a>
            </div>
          )}
        </div>
